...
public class SensorGraphFragment extends Fragment implements SensorEventListener {
    ...
    public void onResume() {
        ...
        if (sensor != null) {
            ...
            if (!registered) {
                valueView.setText("Erreur : Impossible d'enregistrer le listener.");
            }
        } else {
            valueView.setText("Capteur indisponible (" + sensorType + "). Simulation activée.");
            startSimulation();
        }
    }
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.values.length > 0) {
            ...
            android.util.Log.d("SensorDebug", "Sensor: " + sensorType + " Value: " + value);
            updateUi(value);
        }
    }
    ...
    private float extractValue(float[] values) {
        if ("MAGNITUDE".equals(mode) && values.length >= 3) {
    ...
    private void updateUi(float value) {
        ...
        getActivity().runOnUiThread(() -> {
            if (valueView != null) {
                valueView.setText("Valeur détectée : " + String.format("%.2f", value));
            }
            if (chartView != null) {
